...
const Services = () => {
  ...
  const isResettingRef = React.useRef(false); // Track reset state to prevent shaking
  ...
  React.useEffect(() => {
    if (isPaused || selectedPackage || isResettingRef.current) return;
    ...
    const autoScroll = () => {
      ...
      // Check if reached the end and reset smoothly
      if (container.scrollLeft >= maxScrollLeft - 1) {
        isResettingRef.current = true; // Prevent further scrolls during reset
        container.scrollTo({ left: 0, behavior: 'auto' });
        setTimeout(() => {
          isResettingRef.current = false; // Allow scrolling after reset
  ...
  const scrollByCard = (direction) => {
    ...
    // Handle looping without shaking
    setTimeout(() => {
      const maxScrollLeft = container.scrollWidth - container.clientWidth;
      if (container.scrollLeft >= maxScrollLeft - 1 && !isResettingRef.current) {
        isResettingRef.current = true;
        container.scrollTo({ left: 0, behavior: 'auto' });
        setTimeout(() => {
          isResettingRef.current = false;
        }, 50);
      } else if (container.scrollLeft <= 1 && direction < 0 && !isResettingRef.current) {
        isResettingRef.current = true;
